...
pub struct SentenceListGrammar {
    ...
    pub null_result_stop_word: Option<String>,
...
impl SentenceListGrammar {
    ...
    pub fn set_null_result_stop_word<S: Into<String>>(
        &mut self,
        null_result_stop_word: S,
    ) -> &mut Self {
        self.null_result_stop_word = Some(null_result_stop_word.into());
        self.requires_build = true;
        self
    }
...
pub fn grammar_string<T: AsRef<str>>(
    ...
    null_result_stop_word: &Option<T>,
) -> String {
    ...
    match (stop_word_done, null_result_stop_word) {
        (Some(stop_word_done), Some(null_result_stop_word)) => format!(
            "root ::= \" \" ( {range} | \"{}\" ) \" {}\"\n{list_item}",
            null_result_stop_word.as_ref(),
            stop_word_done.as_ref()
        ),
        (None, Some(null_result_stop_word)) => {
            format!(
                "root ::= \" \" ( {range} | \"{}\" )\n{list_item}",
                null_result_stop_word.as_ref()
            )
...
mod tests {
    ...
    #[test]
    fn test_grammar_string_with_stop_word_done_and_null_result_stop_word() {
        let mut grammar = SentenceListGrammar::new();
        grammar
            .min_count(1)
            .max_count(2)
            .set_stop_word_done("done")
            .set_null_result_stop_word("null");
        let expected =
    ...
    #[test]
    fn test_grammar_string_with_null_result_stop_word() {
        let mut grammar = SentenceListGrammar::new();
        grammar
            .min_count(1)
            .max_count(2)
